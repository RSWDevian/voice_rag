"""
Text-to-Speech using local Piper (rhasspy/piper) - v2
Runs a local ONNX voice model fully on-device (no network round trip after the
one-time model download). Same public interface as src/tts.py (get_tts() ->
StreamingTTS with synthesize_text/stream_text/get_performance_stats/close), so
callers only need to change their import from `src.tts` to `src.tts_v2`.
"""
...
# Raw PCM output is resampled to this rate so it matches ElevenLabs'
# pcm_16000 format - the frontend (static/tts_test.html) hardcodes 16kHz
# playback regardless of which TTS engine produced the audio.
...
class PiperTTS:
    ...
    Optimized for low latency by running fully on-device with PCM output.
    """

    def __init__(self):
        """Initialize local Piper TTS voice"""
        self.voice_name = getattr(config, 'PIPER_VOICE', 'en_US-lessac-medium')
        self.use_cuda = getattr(config, 'PIPER_USE_CUDA', False)
        ...
        # Downloads (and caches under models/piper/) the ONNX voice on first
        # run, then loads it into memory for the lifetime of this instance.
        models_dir = Path(config.MODELS_DIR) / "piper"
        models_dir.mkdir(parents=True, exist_ok=True)
        model_path = models_dir / f"{self.voice_name}.onnx"

        if not model_path.exists():
            logger.info(f"Downloading Piper voice '{self.voice_name}'...")
            download_voice(self.voice_name, models_dir)

        self.voice = PiperVoice.load(model_path, use_cuda=self.use_cuda)
        self.native_sample_rate = self.voice.config.sample_rate

        # The underlying ONNX session isn't safe for concurrent calls, so
        # every synthesize() serializes through this lock.
        self._lock = asyncio.Lock()

        logger.info(f"Piper TTS initialized: voice={self.voice_name}, "
                   f"native_rate={self.native_sample_rate}, cuda={self.use_cuda}")

    def _resample(self, audio_int16: np.ndarray, src_rate: int) -> np.ndarray:
        """Resample int16 PCM from the voice's native rate to TARGET_SAMPLE_RATE"""
    ...
    async def synthesize_chunks(self, text: str) -> AsyncGenerator[bytes, None]:
        """
        Synthesize text to audio, yielding one chunk per sentence as Piper
        produces it - Piper's own voice.synthesize() is already a per-sentence
        generator, but a naive wrapper that lists-comprehends it into one
        np.concatenate() call would block until every sentence is done before
        returning any audio at all. Driving the generator one step at a time
        across the executor boundary lets the caller start playback on the
        first sentence while later ones are still being synthesized.

        Args:
            text: Text to synthesize

        Yields:
            bytes: Audio for one sentence at a time
        """
        ...
        syn_config = SynthesisConfig(
            length_scale=(1.0 / self.speed) if self.speed else None
        )
        ...
        try:
            ...
            async with self._lock:
                gen = self.voice.synthesize(text, syn_config=syn_config)
                _sentinel = object()

                while True:
                    audio_chunk = await loop.run_in_executor(None, next, gen, _sentinel)
                    if audio_chunk is _sentinel:
                        break

                    audio = self._resample(audio_chunk.audio_int16_array, self.native_sample_rate)
            ...
            logger.debug(f"TTS synthesis: '{text[:30]}...' ({chunk_count} chunks, "
                       f"{total_bytes} bytes, {latency_ms:.0f}ms)")
    ...
    async def synthesize(self, text: str) -> bytes:
        """
        Synthesize text to audio (non-streaming) - collects synthesize_chunks()
        into one blob for callers that don't need incremental audio.
    ...
    async def close(self):
        """Release the underlying voice model"""
        self.voice = None
...
class StreamingTTS:
    ...
    async def _synthesize_and_emit(self, text: str) -> AsyncGenerator[bytes, None]:
        """Synthesize text via the true per-sentence streaming path, tracking
        stats and firing audio callbacks for each chunk as it's produced."""
        async for audio in self.tts.synthesize_chunks(text):
            self.total_chunks += 1
                ...
                if self.buffer_chars >= self.min_buffer_size:
                    ...
                    if len(text) > self.max_buffer_size:
                        ...
                        if split_point > 0:
                            ...
                            first_part = text[:split_point]
                            async for audio in self._synthesize_and_emit(first_part):
                                yield audio
                            ...
                            async for audio in self._synthesize_and_emit(text):
                                yield audio
...
if __name__ == "__main__":
    ...
    async def test_tts():
        ...
        print("Testing Piper TTS Streaming:")
        print("=" * 60)
